...
def remove_zero_grade_graduates(df: pd.DataFrame) -> pd.DataFrame:
    """Remove graduated students who have 0 approved units on average across both semesters."""
    mask = (
...
class RareCategoryGrouper(BaseEstimator, TransformerMixin):
    """Group categories that appear fewer than `threshold` times under a single label (99)."""
    ...
    def get_feature_names_out(self, input_features=None):
        """This transformer keeps the same columns, so the names do not change."""
...
def load_and_split(file_path: str, test_size: float = 0.2, random_state: int = 42):
    """Load, clean, filter anomalies and split the data."""
